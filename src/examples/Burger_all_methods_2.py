...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov  7 16:56:30 2025
...
def train_one_combo(sampler_name, sampler_kwargs, seed, max_iter=15000):
    ...
    NUM_USED = 5000  # keep in sync with subsampler_kwargs below
    ...
    # --- counts ---
    try:
        # coords/data BEFORE the Subsample_random is applied:
        n_samples_raw = int(ds.get_coords().shape[0])
    except Exception:
        # fallback if API changes
        coords_tmp, data_tmp = partial(create_data, sampler=sampler_name, sampler_kwargs=sampler_kwargs)()
    ...
    meta = {
        ...
        "coeff_vector": [float(c) for c in coeffs],             # raw list for JSON
        "coeffs_dict": {term: float(coeffs[i]) for i, term in enumerate(ORDER)},  # named terms
    }
...
# ---------- Run the grid ----------
# ---------- Run the grid and save both summaries and coefficients ----------
results_csv = "./data/deepymod/burgers/grid_results.csv"
coeffs_csv  = "./data/deepymod/burgers/grid_coeffs.csv"
os.makedirs(os.path.dirname(results_csv), exist_ok=True)

import json, csv

# A) summary CSV (incl. counts + coeff_vector JSON)
    ...
    for s in samplers:
        for kw in grid[s]:
            for sd in seeds:
                ...
                meta = train_one_combo(s, kw, seed=sd, max_iter=25000)
