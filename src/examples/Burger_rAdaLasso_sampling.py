...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 12 11:01:19 2025
...
from GNSINDy.src.deepymod.data.rAdaLasso import RadaLassoSampler
import shutil
...
def create_data():
    """
    Create Burgers data using rAdaLasso sampling.
    Returns:
        coords: torch.float32, shape (m, 2) in (t, x) order
        data:   torch.float32, shape (m, 1)
    """
    # Base grid (torch for solver, but we'll make numpy for sampler)
    x_o_torch = torch.linspace(-8, 8, 100)
    t_o_torch = torch.linspace(0.5, 10.0, 100)
    v, A = 0.1, 1.0

    # Exact can be tensor or numpy; make safe numpy array X of shape (nx, nt)
    _, Exact = burgers_delta_org(x_o_torch, t_o_torch, v, A)
    if isinstance(Exact, torch.Tensor):
        X = Exact.detach().cpu().numpy()
    else:
        X = np.asarray(Exact)

    # 1D numpy grids for the sampler
    x_o = np.linspace(-8, 8, 100)
    t_o = np.linspace(0.5, 10.0, 100)

    # rAdaLasso sampler (tuned to your Burgers setup)
    sampler = RadaLassoSampler(
        X, t_o, x_o,
        num_samples=100,
        poly_order=2, diff_order=2,
        B=40, pi_thr=0.9, EV_max=3,
        random_state=42,
    )
    S_s, T_s, U_s = sampler.execute()  # shapes ~ (m,), (m,), (m,1) or (m,)

    # Ensure 1D arrays
    S_s = np.asarray(S_s).reshape(-1)  # x selection (indices or coords)
    T_s = np.asarray(T_s).reshape(-1)  # t selection (indices or coords)
    U_s = np.asarray(U_s).reshape(-1, 1)

    # If sampler returned *indices*, map them to actual coordinate values
    def maybe_index_to_vals(idx_or_vals, axis_grid):
        a = np.asarray(idx_or_vals)
        if np.issubdtype(a.dtype, np.integer) and a.min() >= 0 and a.max() < len(axis_grid):
            return axis_grid[a]
        return a  # already values
    T_vals = maybe_index_to_vals(T_s, t_o)
    X_vals = maybe_index_to_vals(S_s, x_o)

    # Build tensors in the format DeepMoD expects
    coords = torch.from_numpy(np.stack([T_vals, X_vals], axis=-1)).float()  # (m, 2) -> (t, x)
    data   = torch.from_numpy(U_s.astype(np.float32))                       # (m, 1)
